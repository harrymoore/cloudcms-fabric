...
module.exports = class Script {
    constructor(commandOptions = [], helpMessages = []) {
        ...
        if (this.option_useCredentialsFile) {
            ...
            let rootCredentials = JSON.parse("" + fs.readFileSync(path.join(homeDirectory(), ".cloudcms", "credentials.json")));
    ...
    handleOptions() {
        ...
        if (_.isEmpty(this.options) || this.options.help) {
            this.printHelp(this.mergedOptions);
            return false;
        }
    ...
    printHelp(optionsList) {
        console.log(commandLineUsage([this.helpMessages[0], {
            header: 'Options',
            optionList: this.mergedOptions
        }].concat(this.helpMessages.slice(1))));
    }
